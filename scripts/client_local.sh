...
if [ "$MODEL" == "LINEAR" ]; then
    ...
    pythonw examples/train_linear.py $NWORKERS -r client $VERBOSE -i $TMUX_PANE -t 1
elif [ "$MODEL" == "LOGISTIC" ]; then
    echo "Running Logistic Model"
    pythonw examples/train_logistic.py $NWORKERS -r client $VERBOSE -i $TMUX_PANE -t 1
elif [ "$MODEL" == "V2" ]; then
