...
def linspace_quantization(matrix, interval=8):
    ...
    """
    min_theta_val = matrix.min() + 1e-8
    max_theta_val = matrix.max() + 1e-8
    bins = np.linspace(min_theta_val, max_theta_val, interval)
    theta_bins = np.digitize(matrix, bins).astype(np.int8)
...
def reconstruct_approximation(buf, shape, r_dtype=np.float32):
    ...
    min_theta_val, max_theta_val, interval, matrix_bins = data[0]

    # Generate lineared space vector
    bins = np.linspace(min_theta_val, max_theta_val, interval, dtype=r_dtype)
